...
from src.explanations.face_regions import get_mouth_box
...
def apply_mouth_motion(
    ...
    """
    Replace mouth-region content in frames[start_frame+1 : end_frame] with
    ...
    try:
        ...
        box_s = get_mouth_box(lm_start, w, h)
        box_e = get_mouth_box(lm_end, w, h)
        ...
        if target_size[0] < 1 or target_size[1] < 1:
            raise RuntimeError("Mouth bounding box is too small to interpolate.")
        ...
        for idx in range(start_frame + 1, end_frame):
            ...
            output[idx] = blend_region(output[idx], interpolated, box_s, feather_ratio)
